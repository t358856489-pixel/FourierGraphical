...
**测试**: 项目章程(原则 III"测试先行")要求包含测试任务: 每个故事的测试任务必须排在实现任务之前, 先写并确认失败.
...
### 用户故事 1 的测试(章程原则 III 要求 - 先写并确认失败)⚠️
...
### 用户故事 2 的测试(章程原则 III 要求 - 先写并确认失败)⚠️
...
### 用户故事 3 的测试(章程原则 III 要求 - 先写并确认失败)⚠️
